...
/**
 * 利用属性拷贝工具，消除重复代码逻辑
 * 对于三层架构来说，考虑到层之间的解耦隔离以及每一层对数据的不同需求，通常每一层都会有自己的 POJO 作为数据实体。
 * 比如：数据访问层的实体一般叫做 DataObject 或 DO;
 *      业务逻辑层的实体一般叫做 Domain;
 *      表现层的实体一般叫做 Data Transfer Object 或 DTO。
 */
@RestController
...
public class BeanCopyController {
    ...
    /**
     * 使用 BeanUtils 这种 Mapping 工具来做 Bean 的转换，copyProperties 方法还允许我们提供需要忽略的属性。
     */
